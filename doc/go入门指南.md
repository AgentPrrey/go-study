...
# 6.2 函数参数和返回值
函数接收参数给自己用，也可以返回一个或多个值

关键字return 用于返回一组值

函数定义时，形参一般有名字 

也可以定义没有形参名，只有形参类型的函数

按值传递，传递的是参数的副本，不会影响原来的变量

按引用传递，将参数地址传递给函数，通过指针的值来修改指向地址上的值（消耗更少,可以节省内存）
...
命名返回值作为结果形参被初始化为相应类型的零值，当需要返回的时候，我们只需要一条简单的不带参数的 return 语句

空白符_用于匹配一些不需要的值，然后将其丢弃

传递指针给了函数直接修改外部变量的能力，所以被修改的变量不再需要使用 return 返回

# 6.3 传递变长函数
函数的最后一个参数采用...type 的形式，个函数就可以处理一个变长的参数
